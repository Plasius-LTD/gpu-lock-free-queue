# Contributing to @plasius/gpu-lock-free-queue
...
`@plasius/gpu-lock-free-queue` provides a small, fast lock-free MPMC ring queue for WebGPU:

- WGSL queue implementation with per-slot sequence counters (ABA-resistant within a 32-bit epoch).
- Minimal JS helper to load the WGSL.
- Demo that enqueues/dequeues large batches and visualizes FFT randomness metrics.
...
- `fix: correct queue slot sequence update`
- `docs: clarify job payload shape guidance`
- `refactor: tighten WGSL retry loop`
